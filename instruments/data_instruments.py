...
class DataInstruments(Resources):
    ...
    def check_duplicates(self):
        ...
        with open("dublicates_log.txt", "w", encoding="utf-8") as file:
            ...
            for row in range(1, self.models_sheet.max_row + 1):
                name = self.models_sheet.cell(row, 2).value
                name_add = self.models_sheet.cell(row, 5).value
    ...
    def get_mark(self, row):
        for i in range(50, 85):
            if self.export_sheet.cell(row, i).value == "Марка":
                return self.export_sheet.cell(row, i + 2).value

    def get_colour(self, row):
        for i in range(50, 85):
            if self.export_sheet.cell(row, i).value == "Цвет":
                return self.export_sheet.cell(row, i + 2).value
    ...
    def get_all_marks(self):
        ...
        for row in range(2, self.export_sheet.max_row + 1):
            for i in range(50, 85):
                cell = self.export_sheet.cell(row, i).value
                if cell == "Марка":
                    mark = self.export_sheet.cell(row, i + 2).value
    ...
    def get_groups_data(self):
        ...
        for row in range(1, self.groups_sheet.max_row + 1):
            group_id = self.groups_sheet.cell(row, 1).value
            name = self.groups_sheet.cell(row, 2).value
            groups.update([(name, group_id)])
        return groups

    def fulfill_groups_data(self, groups_data, parent_group):
        ...
        if parent_group:
            for row in range(3, len(group_ids) + 2):
                self.blank_groups.cell(row, 6).value = group_ids[0]

        for i in range(len(group_names)):
            self.blank_groups.cell(i + 2, 2).value = group_names[i]
            self.blank_groups.cell(i + 2, 4).value = group_ids[i]
